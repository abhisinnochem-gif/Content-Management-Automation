"""
Country Life Vitamins Image Downloader — Plugin for download_webp.py
=====================================================================
Exact same default flow as original download_webp.py:
  - Search by SKU then UPC, validate with BAD_SLUGS
  - Find gallery container, match images by SKU number prefix (e.g. 3051_)
    then SKU/UPC/name tokens
  - Remove BG, crop, resize to 4 sizes, save as JPEG

Usage:
    python countrylife_downloader.py --sku 3051 --upc 015794030515 --name "Dairy Free Acidophilus" --folder resized_images/3051
"""
...
CL_DOMAIN  = "countrylifevitamins.com"
BASE_URL   = "https://countrylifevitamins.com"
...
SKIP = [
    ...
    "open-bottle", "_copy", "claims", "lifestyle",
...
# NOTE: "yesno" intentionally NOT in SKIP for Country Life —
# the old script output shows 3051_YES_NO.jpg was downloaded as a valid product image
...
def get_all_webp_urls(product_url, sku, upc, product_name):
    ...
    # Country Life uses SKU number as filename prefix e.g. 3051_
    # Build vendor prefix from SKU
    sku_prefix = str(sku).strip() + "_" if sku else ""
    vendor_prefixes = [sku_prefix] if sku_prefix else []
    ...
    for fname, (cdn_base, w) in filenames.items():
        ...
        # Priority 1: SKU number prefix (e.g. 3051_)
...
def process_excel(filepath):
    ...
    for row in ws.iter_rows(min_row=4, max_row=ws.max_row):
        ...
        if not url or CL_DOMAIN not in url: continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Country Life Vitamins Image Downloader")
    ...
    if args.sku and args.folder:
        ...
        print("Usage: countrylife_downloader.py --sku 3051 --upc 015794030515 --name 'Dairy Free Acidophilus' --folder resized_images/3051")
        sys.exit(1)
